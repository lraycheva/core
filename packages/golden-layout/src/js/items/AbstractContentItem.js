...
lm.items.AbstractContentItem = function (layoutManager, config, parent) {
	...
	// This property is added in order the patch the issue which is present when a child has been replaced and the active content item is changed
	// When you change the selected tab when a new child participates the hide method is invoked on the old child and the new doesn't here the event
	// This flag is used to indicate that and to change the visibility of the new child
	this.isVisible = true;
...
lm.utils.copy(lm.items.AbstractContentItem.prototype, {
	...
	replaceChild: function (oldChild, newChild, _$destroyOldChild) {
		...
		const isOldChildVisible = oldChild.isVisible;
		/*
	...
	getComponentsByName: function (componentName) {
		...
		for (i = 0; i < components.length; i++) {
			// Preserve the Gl logic and if instance is something meaningful return instance however if instance is just an empty object or undefined return the component
			// In theory this is the correct behaviour, there is no mention of an instance in the docs
			instances.push(components[i].instance && Object.keys(components[i].instance).length !== 0 ? components[i].instance : components[i]);
		}
