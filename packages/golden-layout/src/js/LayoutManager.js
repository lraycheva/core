...
lm.utils.copy(lm.LayoutManager.prototype, {
	...
	toConfig: function (root) {
		...
		config = {
			...
			context: lm.utils.copy({}, this.config.context),
			workspacesOptions: lm.utils.copy({}, this.config.workspacesOptions)
		};
		 ...
		 * Add maximised item
		 */
		config.maximizedItemId = this._maximizedItem ? '__glMaximised' : null;
		return config;
	},
	...
	destroy: function () {
		...
		this._dragSources.forEach(function (dragSource) {
			if (typeof dragSource._dragListener.destroy === "function") {
	 ...
	 * @returns {void}
	 */
	createDragSource: function (element, itemConfig, customDragListener) {
		this.config.settings.constrainDragToContainer = false;
		var dragSource = new lm.controls.DragSource($(element), itemConfig, this, customDragListener);
	...
	_$createRootItemAreas: function () {
		...
		for (var side in sides) {
			...
			else if (side === "y2") { // Added to enable proper top drop area and proper tab drop area
				area[side] = areaSize + this.root.element.offset().top;
			} else {
				area[side] = areaSize;
			}
	...
	_create: function (config) {
		...
		if (config.maximizedItemId === '__glMaximised') {
			this.root.getItemsById(config.maximizedItemId)[0].toggleMaximise();
		}
	},
